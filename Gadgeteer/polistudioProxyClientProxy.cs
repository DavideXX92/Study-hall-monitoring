//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//     .NET Micro Framework MFSvcUtil.Exe
//     Runtime Version:2.0.00001.0001
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------


using System;
using System.Xml;
using Dpws.Client;
using Dpws.Client.Discovery;
using Dpws.Client.Eventing;
using Ws.Services;
using Ws.Services.Utilities;
using Ws.Services.Binding;
using Ws.Services.Soap;
using Ws.Services.WsaAddressing;
using Ws.Services.Xml;
using Microsoft.SPOT;

namespace polistudio
{
    public class IServiceClientProxy : DpwsClient
    {
        
        private IRequestChannel m_requestChannel = null;

        public IServiceClientProxy(Binding binding, ProtocolVersion version) : 
                base(binding, version)
        {

            // Set client endpoint address
            m_requestChannel = m_localBinding.CreateClientChannel(new ClientBindingContext(m_version));
        }
        
        public virtual loginResponse login(login req)
        {

            // Create request header
            String action;
            action = "http://polistudio/IService/login";
            WsWsaHeader header;
            header = new WsWsaHeader(action, null, EndpointAddress, m_version.AnonymousUri, null, null);
            //header = new WsWsaHeader(null, null, null, null, null, null);
            WsMessage request = new WsMessage(header, req, WsPrefix.None);
            
            // Create request serializer
            loginDataContractSerializer reqDcs;
            reqDcs = new loginDataContractSerializer("login", "http://polistudio/");
            request.Serializer = reqDcs;
            request.Method = "login";


            // Send service request
            m_requestChannel.Open();
            WsMessage response = response = m_requestChannel.Request(request);
            m_requestChannel.Close();

            // Process response
            loginResponseDataContractSerializer respDcs;
            respDcs = new loginResponseDataContractSerializer("loginResponse", "http://polistudio/");
            loginResponse resp;
            resp = ((loginResponse)(respDcs.ReadObject(response.Reader)));
            response.Reader.Dispose();
            response.Reader = null;
            return resp;
        }
        
        public virtual portRequestResponse portRequest(portRequest req)
        {

            // Create request header
            String action;
            action = "http://polistudio/IService/portRequest";
            WsWsaHeader header;
            header = new WsWsaHeader(action, null, EndpointAddress, m_version.AnonymousUri, null, null);
            WsMessage request = new WsMessage(header, req, WsPrefix.None);

            // Create request serializer
            portRequestDataContractSerializer reqDcs;
            reqDcs = new portRequestDataContractSerializer("portRequest", "http://polistudio/");
            request.Serializer = reqDcs;
            request.Method = "portRequest";


            // Send service request
            m_requestChannel.Open();
            WsMessage response = m_requestChannel.Request(request);
            m_requestChannel.Close();

            // Process response

            portRequestResponseDataContractSerializer respDcs;
            respDcs = new portRequestResponseDataContractSerializer("portRequestResponse", "http://polistudio/");
            portRequestResponse resp;
            resp = ((portRequestResponse)(respDcs.ReadObject(response.Reader)));
            response.Reader.Dispose();
            response.Reader = null;
            return resp;
        }
        
        public virtual logoutResponse logout(logout req)
        {

            // Create request header
            String action;
            action = "http://polistudio/IService/logout";
            WsWsaHeader header;
            header = new WsWsaHeader(action, null, EndpointAddress, m_version.AnonymousUri, null, null);
            WsMessage request = new WsMessage(header, req, WsPrefix.None);

            // Create request serializer
            logoutDataContractSerializer reqDcs;
            reqDcs = new logoutDataContractSerializer("logout", "http://polistudio/");
            request.Serializer = reqDcs;
            request.Method = "logout";


            // Send service request
            m_requestChannel.Open();
            WsMessage response = m_requestChannel.Request(request);
            m_requestChannel.Close();

            // Process response
            logoutResponseDataContractSerializer respDcs;
            respDcs = new logoutResponseDataContractSerializer("logoutResponse", "http://polistudio/");
            logoutResponse resp;
            resp = ((logoutResponse)(respDcs.ReadObject(response.Reader)));
            response.Reader.Dispose();
            response.Reader = null;
            return resp;
        }
    }
}
